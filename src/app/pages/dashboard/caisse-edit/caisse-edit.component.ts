  ...
  getCaisse(id : any){
    this.httpService.authGet(AppConstants.caisse+`/${id}`).subscribe({
      next : (data : any) => {
        ...
        this.caisse.operationDate = new Date(data.body.operationDate.substring(0,10))

        this.form.patchValue({
          libelle : this.caisse.libelle,
          amountIn : this.caisse.amountIn,
          amountOut : this.caisse.amountOut,
          operationDate : this.caisse?.operationDate?.toISOString().substring(0,10)
        })
      } ,
      error : (error) => {
    ...
    this.httpService.authPut(`${AppConstants.caisse}/${this.id}`, data)
      .subscribe({
        next: (resp : any) => {
          ...
          this.toastService.showSuccessToast("Succes !" , "L'opération s'est bien modifiée");
